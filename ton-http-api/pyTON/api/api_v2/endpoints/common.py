...
@router.get(
    "/getConfigParam",
    response_model=GetConfigParamResponse,
    response_model_exclude_none=True,
    tags=["get config"],
)
...
@router.get(
    "/detectAddress",
    response_model=TonResponse200Generic[AddressForms],
    responses=get_get_address_information_error_responses(),
    response_model_exclude_none=True,
    tags=["accounts"],
)
    ...
    if res.get("@type") == "ok":
        ...
        if settings.webserver.boc_endpoint is not None:
            background_tasks.add_task(
                send_boc_to_external_endpoint, base64.b64encode(boc).decode("utf8")
            )
    return res
    ...
    for i in range(int(send_duration / send_interval)):
        try:
            ...
            if res.get("@type") == "ok":
                ...
                if settings.webserver.boc_endpoint is not None:
                    send_boc_to_external_endpoint(
                        base64.b64encode(boc_bytes).decode("utf8")
                    )
        except:
            pass
...
if settings.webserver.get_methods:
    ...
    @router.post(
        "/runGetMethod",
        response_model=RunGetMethodResponse,
        response_model_exclude_none=True,
        tags=["run method"],
    )
