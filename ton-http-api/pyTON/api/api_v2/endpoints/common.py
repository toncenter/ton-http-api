...
if settings.webserver.json_rpc:
    @router.post('/jsonRPC', response_model=DeprecatedTonResponseJsonRPC, response_model_exclude_none=True, tags=['json rpc'])
    async def jsonrpc_handler(json_rpc: TonRequestJsonRPC, 
        ...
        return DeprecatedTonResponseJsonRPC(ok=result.ok, result=result.result, error=result.error, code=result.code, id=_id)
