...
@app.get('/getRawBlockTransactions', response_model=TonResponse, response_model_exclude_none=True, tags=['blocks','transactions'])
@json_rpc('getRawBlockTransactions')
@wrap_result
async def get_raw_block_transactions(
    workchain: int,
    shard: int,
    seqno: int,
    root_hash: Optional[str] = None,
    ...
    Get transactions of the given block.
    """
    block_txs = await tonlib.getBlockTransactions(workchain, shard, seqno, count, root_hash, file_hash, after_lt, after_hash)
    requests = []
    for tx in block_txs['transactions']:
        requests.append(tonlib.get_transactions(tx['account'], from_transaction_lt=tx['lt'], from_transaction_hash=tx['hash'], to_transaction_lt=0, limit=1, archival=False))
    r = await asyncio.gather(*requests)
    for i in range(len(block_txs['transactions'])):
        tx = block_txs['transactions'][i]
        tx['raw'] = r[i][0]
    return block_txs
